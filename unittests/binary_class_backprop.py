...
model = ConvGramClassifier(
    vocab=[tok for tok, _ in vocab.most_common(10000)],
    embedding_size=64,
    conv_filter_size=16,
    ngrams=[1, 2, 3],
    pool_size=3,
...
# svm_platt_binary_train_step(model, optimizer, tokens=tf.ragged.constant(x_train[:16]), labels=y_train[:16])
...
for i, b in enumerate(range(0, len(x_train) + batch_size, batch_size), start=1):
    ...
    if i % 10 == 0:
        print(f"SVM Loss: {svm_loss.result()} Platt posterior Loss: {platt_loss.result()}")
